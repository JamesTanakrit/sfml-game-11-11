...
public:
	...
	{
		deltaTime = 0;
		maptexture.loadFromFile("newMap2.png");
	...
	inline void update(RenderWindow& window)
	{
        ...
        if (enemies.size() < EnemyNumMax && enemyRate >= enemyRateMax)
        {
            ...
            if (enPos == 1)
            {
                enemies.push_back(Enemy(Vector2f(40, 40), Vector2f(0, randint(228, 692)), enSpd));
            }
            if (enPos == 2)
            {
                enemies.push_back(Enemy(Vector2f(40, 40), Vector2f(1240, randint(228, 692)), enSpd));
            }
            if (enPos == 3)
            {
                enemies.push_back(Enemy(Vector2f(40, 40), Vector2f(randint(303, 937), 0), enSpd));
            }
            if (enPos == 4)
            {
                enemies.push_back(Enemy(Vector2f(40, 40), Vector2f(randint(303, 937), 920), enSpd));
            }
